...
function createNativeChatApi(): NativeChatApi {
  return {
    // [FORK] Disk session-discovery is a desktop-local fallback (IPC-only, no
    // runtime RPC bridge). The web client has no local transcripts to scan, so
    // it skips the fallback and relies on the live provider session / readSession.
    discoverCursorSession: () => Promise.resolve(null),
    discoverClaudeSession: () => Promise.resolve(null),
